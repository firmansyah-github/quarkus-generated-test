...
if [ -f "$jar_path" ]; then
    ...
    java -jar "$jar_path" > postman.service.log &
    SERVICE_PROCESS=$!
    tail -f -n0 postman.service.log | grep -q 'Listening on'
    echo "Application started"
    ./src/main/postman/run.api.tests.sh    # Modify this line to provide the correct path if needed
    kill $SERVICE_PROCESS
    rm postman.service.log
else
