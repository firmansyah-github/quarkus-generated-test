...
if [ -f "$jar_path" ]; then
    ...
    java -jar "$jar_path" > zap.service.log &
    SERVICE_PROCESS=$!
    tail -f -n0 zap.service.log | grep -q 'Listening on'
    echo "Application started"
    ./src/main/zap/run.api.tests.sh    # Modify this line to provide the correct path if needed
