...
if [ -f "$jar_path" ]; then
    ...
    java -jar "$jar_path" > k6.service.log &
    SERVICE_PROCESS=$!
    tail -f -n0 k6.service.log | grep -q 'Listening on'
    echo "Application started"
    ./src/main/k6/run.api.tests.sh    # Modify this line to provide the correct path if needed
