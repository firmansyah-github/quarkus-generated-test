...
@Path("/firmansyah/region")
@AllArgsConstructor
public class RegionResource {  
  
    private final FindRegionByFilter findRegionByFilter;
  	...
  	private final RegionResourceUtils resourceUtils;
  	
  	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	@Secured(optional = true)
	public Response findRegionByFilter(@QueryParam("offset") int offset, @QueryParam("limit") int limit,
		...
		final var regionPageResult = findRegionByFilter.handle(resourceFilter);

		return Response.ok(
            objectMapper.writeValueAsString(
                resourceUtils.regionResponse(regionPageResult)))
	...
	@Path("/find")
  	@Secured(optional = true)
  	@Produces(MediaType.APPLICATION_JSON)
  	public Response findRegionByPrimaryKey(@Context SecurityContext securityContext ,
			@QueryParam("regionId") Integer regionId) {
    	final var region = findRegionByPrimaryKey.handle(regionId);
    	
    	return Response.ok(resourceUtils.regionResponse(region)).status(Response.Status.OK).build();
  	}
    ...
    final var region = createRegion.handle(newRegionRequest.toNewRegionInput());
    return Response.ok(resourceUtils.regionResponse(region))  
  	...
  	public Response update(
      		@Valid @NotNull UpdateRegionRequest updateRegionRequest,
      		@Context SecurityContext securityContext) {
    	final var updatedRegion =
        	updateRegion.handle(updateRegionRequest.toUpdateRegionInput());
    	return Response.ok(resourceUtils.regionResponse(updatedRegion))
        	.status(Response.Status.OK)
        	.build();
  	}
  	...
  	public Response delete(
		...
		if (deleteRegion.handle(regionId)) {
